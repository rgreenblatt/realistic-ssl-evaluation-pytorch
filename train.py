...
print("labeled data : {}, unlabeled data : {}, training data : {}".format(
    ...
    len(l_train_dataset) + len(u_train_dataset)))
print("validation data : {}, test data : {}".format(len(val_dataset),
                                                    len(test_dataset)))
...
if args.alg != "supervised":
    ...
    l_loader = DataLoader(l_train_dataset,
                          ...
                          sampler=RandomSampler(
                              len(l_train_dataset), shared_cfg["iteration"] *
                              shared_cfg["batch_size"] // 2))
else:
    l_loader = DataLoader(l_train_dataset,
...
for l_data, u_data in zip(l_loader, u_loader):
    ...
    if args.alg != "supervised":  # for ssl algorithm
        ...
        u_input, dummy_target = u_input.to(device).float(), dummy_target.to(
            device).long()
        ...
        with torch.no_grad():
            ...
            for j, data in enumerate(val_loader):
                ...
                if ((j + 1) % 10) == 0:
                    ...
                    print("[{}/{}] time : {:.1f} data/sec, rest : {:.2f} sec".
                ...
                for j, data in enumerate(test_loader):
                    ...
                    if ((j + 1) % 10) == 0:
                        ...
                        print(
                            ...
                            .format(j + 1, len(test_loader), d_p_s,
                                    (len(test_loader) - j - 1) / d_p_s),
                            "\r",
                            end="")
                        s = time.time()
                print()
